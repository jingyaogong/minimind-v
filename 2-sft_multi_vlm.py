...
from model.dataset import SFTDataset_multi
...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask, image_process) in enumerate(train_loader):
        ...
        image_encoders = get_img_embedding(image_process, vision_model) # torch.Size([32, 4, 50, 768])
    ...
    if args.save_last and (not ddp or dist.get_rank() == 0):
        model.eval()
        moe_path = '_moe' if lm_config.use_moe else ''
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="./dataset/sft_multi_images/output_cn.json",
    ...
    use_version = 0
    train_ds = SFTDataset_multi(args.data_path, tokenizer, vision_model=(vision_model, preprocess),
